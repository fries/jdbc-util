 ...
 */
public class StatementInParametersTest
{
    ...
    {
        Object[] result = statementUtil.selectInto(
                "SELECT @array",
                ResultSetReaders.readOne(),
                ResultSetMappers.tuple(
    ...
    private <T> void check(ResultType<T> resultType, StatementInParameterType<T> statementParameter, T value) throws Exception
    {
        assertEquals( value, statementUtil.selectInto(
                "SELECT ?",
                ResultSetReaders.readOne(),
                ResultSetMappers.scalar( resultType ),
                bindValue( statementParameter, value )
        ) );
    }

    @SuppressWarnings("unchecked")
    private <T> void check(ResultType<T> resultType, StatementInParameterType<T> statementParameter, T value1, T value2) throws Exception
    {
        Object[] result = statementUtil.selectInto(
                "SELECT @array",
                ResultSetReaders.readOne(),
                ResultSetMappers.tuple( resultType, resultType ),
